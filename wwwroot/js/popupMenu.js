...
export function colorPopup(lobbyConnection, player, colors, associatedPlayers) {
    const title = "Color selection";
    const id = "color-popup-menu";
    const content = colorContent(lobbyConnection, player, colors, associatedPlayers);

    const popup = createPopup(title, id, content);

    popupMenuContainer.innerHTML = "";
    popupMenuContainer.append(popup);
}

function colorContent(connection, player, colors, associatedPlayers) {
    ...
    for (let i = 0; i < colors.length; i++) {
        ...
        span.addEventListener("click", (e) => {
            connection.invoke("ChangeColor", player.privateId, i).catch((err) => {
...
function createPopup(title, id, content) {
